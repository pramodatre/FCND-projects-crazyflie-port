...
def create_grid_crazyflie(data, north_size, east_size,
    ...
    for i in range(data.shape[0]):
        ...
        if alt + d_alt + safety_distance > drone_altitude:
            obstacle = [
                int(np.clip(north - d_north - safety_distance, 0, north_size - 1)),
                int(np.clip(north + d_north + safety_distance, 0, north_size - 1)),
                int(np.clip(east - d_east - safety_distance, 0, east_size - 1)),
                int(np.clip(east + d_east + safety_distance, 0, east_size - 1)),
            ]
            grid[obstacle[0]:obstacle[1] + 1, obstacle[2]:obstacle[3] + 1] = 1
...
def create_graph_and_edges_crazyflie(data, north_size, east_size,
                                     drone_altitude, safety_distance):
    # Initialize an empty grid
    grid = np.zeros((north_size, east_size))
...
def find_start_goal(graph, start, goal):
    ...
    for n in graph:
        # print(n)
        # print(np.array(n), np.array(start))
        # print(np.array(n), np.array(goal))
...
def condense_waypoints_crazyflie(grid, path):
    """
    Apply Bresenham over all points in the path and retain
    minimal waypoints. Will retain fisrt and last waypoint.

    Test that is failing: 
    Input: [(32, 40, 0), (33, 33, 7), (24, 69, 2), (55, 58, 8), (80, 80, 8), (102, 81, 6), (94, 51, 3), (111, 29, 3), (100, 25, 0)]
    Reduced path: [(32, 40), (80, 80), (80, 80), (80, 80), (80, 80), (100, 25)] => collides with obstacles!
    ...
    Returns:
        (list): containing munimal points from start to end point
                in path
    """
    print(f'Path to be reduced: {path}')
    sampled_waypoints = []
    i = 0
    while i < len(path):
        # first point is added
        if i == 0:
            sampled_waypoints.append(path[i])
        else:
            # check if we can connet last point we saved and the i th point
            p1 = sampled_waypoints[-1]
            p = path[i]
            cells = list(bresenham(int(p1[0]), int(p1[1]),
                                   int(p[0]), int(p[1])))
            ...
            for c in cells:
                if grid[c[0], c[1]] == 1:
                    hit = True
            if hit:
                sampled_waypoints.append(path[i - 1])
                hit = False
    ...
    # Ensure last point in the path is in the condensed path
    p_last = path[-1]
    if not p_last in sampled_waypoints:
        sampled_waypoints.append(p_last)
...
def extract_polygons_crazyflie(data, safety_distance=None):
    polygons = []
    for i in range(data.shape[0]):
        north, east, alt, d_north, d_east, d_alt = data[i, :]
        ...
        # corners = [None, None, None, None]
        corners = [
            (north - d_north - safety_distance, east - d_east - safety_distance),
            (north - d_north - safety_distance, east + d_east + safety_distance),
            (north + d_north + safety_distance, east + d_east + safety_distance),
            (north + d_north + safety_distance, east - d_east - safety_distance)
...
def collides_kdtree_crazyflie(polygons, tree, point):
    # TODO: Determine whether the point collides
    # with any obstacles.
    x, y, z = point
    p = Point(x, y)
...
def construct_road_map_crazyflie(data, north_size, east_size,
                                 drone_altitude, safety_distance,
                                 num_nodes, neighbors):
    grid = create_grid_crazyflie(data, north_size, east_size,
                                 drone_altitude, safety_distance)
    print("Completed grid construction...")
    polygons = extract_polygons_crazyflie(data, safety_distance)
    poly_bounds = []
    for poly, height in polygons:
        poly_bounds.append(np.array(list(poly.centroid.coords))[0])
    print("Completed polygon creation for obstacles...")
    ...
    while len(to_keep) < num_nodes:
        ...
        # Check for collission free nodes
        for point in samples:
            if not collides_kdtree_crazyflie(polygons, tree_poly, point):
                to_keep.append(point)
    print("Completed node selection...")
    # randomly sample nodes from to_keep, find neighbors
    # and add edges when fesible
    ...
    while len(g.edges) < num_nodes:
        ...
        nodes = []
        for point in nodes_sampled:
            nodes.append([point[0], point[1], point[2]])
        tree = KDTree(np.array(nodes))
        # print("KDTree created for the sampled nodes...")
        for n in nodes:
            # print("Querying the KKDTree...")
            ...
            for i in idx:
                ...
                # print("Checking can_connect() ...")
                # Too slow on the simulator!
                if can_connect(polygons, Point(n), Point(n_i)):
                    g.add_edge(tuple(n), tuple(n_i), weight=1)
                    edges[tuple(n)] = tuple(n_i)
                    edges[tuple(n_i)] = tuple(n)
                    # print("can be connected.")
...
def visualize_prob_road_map(data, grid, g):
    ...
    for (n1, n2) in g.edges:
        plt.plot([n1[1] - emin, n2[1] - emin],
                 [n1[0] - nmin, n2[0] - nmin], 'black', alpha=0.5)
...
def visualize_prob_road_map_crazyflie(data, grid, g, start=None,
                                      goal=None, path=None,
                                      all_nodes=False):
    fig = plt.figure()
    plt.imshow(grid, cmap='Greys', origin='lower')
    ...
    plt.xlabel('EAST')
    plt.ylabel('NORTH')
    if start is not None and goal is not None and path is not None:
...
if __name__ == '__main__':
    ...
    # dir_motion_planning = '/Users/pramod/dev/bitbucket/autonomous-flight-engineer-nano-degree/FCND-crazyflie/motion_planning'
    data = np.loadtxt('map_obstacle_course.csv', delimiter=',',
                      dtype='Float64', skiprows=2)
    TARGET_ALTITUDE = 0.3 / 0.0254
    SAFETY_DISTANCE = 0.10 / 0.0254
    st = time.time()
    # grid, G = create_graph_and_edges_crazyflie(
    #     data, 130, 86, TARGET_ALTITUDE, SAFETY_DISTANCE)
    grid, G = construct_road_map_crazyflie(data, 130, 86,
                                           TARGET_ALTITUDE,
                                           SAFETY_DISTANCE, 100, 4)
    time_taken = time.time() - st
    ...
    # unique_path = [(32, 40, 0), (33, 33, 7), (24, 69, 2), (55, 58, 8),
    #                (80, 80, 8), (102, 81, 6), (94, 51, 3), (111, 29, 3), (100, 25, 0)]
    ...
    #     if i == 0:
    #         n_prev, e_prev, _ = point
    #     else:
    #         n, e, _ = point
    #         # print(n - n_prev, e - e_prev, TARGET_ALTITUDE * 0.0254)
    #         waypoints.append([(n - n_prev) * 0.0254,
    #                           (e - e_prev) * 0.0254,
    #                           TARGET_ALTITUDE * 0.0254])

    # print(path)
    # Convert path to waypoints
    # waypoints = [[p[0] * 0.0254, p[1] * 0.0254,
    #               TARGET_ALTITUDE * 0.0254] for p in path]
    # waypoints_as_offsets = []
    # for i in range(len(waypoints) - 1):
    #     print(list(np.array(waypoints[i+1]) - np.array(waypoints[i])))
    #     offsets = list(np.array(waypoints[i+1]) - np.array(waypoints[i]))
    #     offsets[2] = TARGET_ALTITUDE * 0.0254
    #     waypoints_as_offsets.append(offsets)
    # Set self.waypoints
